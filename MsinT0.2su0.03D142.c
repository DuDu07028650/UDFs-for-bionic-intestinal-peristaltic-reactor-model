...
#define  A   0.002        /* amplitude of the contraction, meters*/
...
DEFINE_GRID_MOTION(segmentation_upperwall,domain,dt,time,dtime)
{
	...
	real F=0.0;
	int t_now;
	t_now = RP_Get_Integer("time-step");
	int n;
	int m = 24;
			...
			if(NODE_POS_NEED_UPDATE(node_p))    /* 判断格点是否已经更新:否为1，是为0*/
				{
					...
					F= sin(2*M_PI*x/L);
					NODE_Y(node_p) = 0.006-A*sin(2*M_PI/T*t_now)*F;    /* 修改y坐标 */
			...
			if(NODE_POS_NEED_UPDATE(node_p))    /* 判断格点是否已经更新:否为1，是为0*/
				{
					...
					x=NODE_X(node_p);           /*  获取node格点x坐标 */
					F=sin(2*M_PI*x/L);;
					NODE_Y(node_p) = A*sin(2*M_PI/T*t_now)*F;    /* 修改y坐标 */
